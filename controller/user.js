...
exports.profile = async (req, res, next) => {
  ...
  let User = await user
    .find({ _id: uid })
    .select({ name: 1, email: 1, phone: 1, collegeId: 1 });
  if (User.length > 0) {
    User = User[0];
    res.render("profile.ejs", {
      username: User.name,
  ...
  (exports.updatePassword = async (req, res, next) => {
    try {
      ...
      if (matched) {
        res.render("changepassword.ejs", {
          title: "change password",
          err: "new password should not be the old password",
        });
      }
...
exports.confirmMail = async (req, res, next) => {
  ...
  if (User.length > 0) {
    ...
    if (mailed)
      res.render("forgotpwd1.ejs", {
        title: "forgot password",
        err: "A mail is sent to change password",
      });
